...
permalink: /1.21.x/item-stack-modifiers/
---
...
- [Crafting Remainder](#crafting-remainder)
- [Damage Crafting Remainder](#damage-crafting-remainder)
- [Dye Leather](#dye-leather)
- [Empty Bowl](#empty-bowl)
- [Extra Product](#extra-product)
- [Meal](#meal)
- [Remove Dye](#remove-dye)
- [Remove Trait](#remove-trait)
- [Reset Food](#reset-food)
- [Round Creation Date](#round-creation-date)
...
- `type`: `tfc:add_heat`
- `temperature`: A number. An amount in degrees Celsius to be added.
...
This modifier applies a chance for the output item to be produced. Each item in the stack is rolled individually. It has the following fields:

- `type`: `tfc:chance`
- `chance`: A float in the range [0, 1], representing the probability that each item will be produced.
...
This modifier returns the crafting remainder (container item) of the input item, if it has one. For example, using a bucket of water in a recipe would return an empty bucket. It has the following fields:

- `type`: `tfc:crafting_remainder`

**This modifier is only usable in crafting recipes which support item stack providers.**

<hr>

## Damage Crafting Remainder

This modifier damages the input item by 1 durability and returns it as the crafting remainder. If the item breaks, it returns empty. This is useful for recipes that consume tool durability. It has the following fields:
...
This modifier applies a dye color to a dyeable item (such as leather armor). It has the following fields:

- `type`: `tfc:dye_leather`
- `color`: A dye color. Must be one of: `white`, `orange`, `magenta`, `light_blue`, `yellow`, `lime`, `pink`, `gray`, `light_gray`, `cyan`, `purple`, `blue`, `brown`, `green`, `red`, `black`.
...
This modifier gives an additional item to the player when crafting. The extra product is given directly to the player's inventory (or dropped if full). It has the following fields:

- `type`: `tfc:extra_products`
- `stack`: An [Item Stack](../common-types/#item-stacks). The extra item to be given to the player.
...
```jsonc
// Reference: data/tfc/recipes/crafting/oat_bread_sandwich_sss.json
{
    "type": "tfc:meal",
    ...
    "portions": [
        {
            ...
            // anything that matches the ingredient will get their values multiplied by 0.675 and added to the sandwich
            "ingredient": {
              "item": "tfc:food/oat_bread"
            },
            "nutrient_modifier": 0.675,
...
This modifier removes the dye color from a dyed item, returning it to its default color. It has the following fields:
...
This modifier rounds the food creation date to the nearest day. This is useful for standardizing food timestamps. It has the following fields:
