...
A **Rock Setting** is a collection of mappings of a name to the resulting blocks that are used for that rock. It can be either a name (the ID of a rock setting defined at `/data/<namespace>/tfc/worldgen/rock_settings/<path>.json`) or an inline rock setting object.

Rock settings are defined as an object with the following fields:
...
- `spike`, `loose`, `mossy_loose` (Optional String) These are names of blocks that are used for this rock in the respective places in world generation, but are optional, and won't generate if not present.
- `karst` and `mafic` (Optional Boolean) if these rocks are mafic or karst types.

##### Example

```jsonc
// The default basalt rock setting used by TFC, found at
// data/tfc/tfc/worldgen/rock_settings/basalt.json
{
...
- `rocks` is an object which is a map of **Rock** IDs, to [Rock Setting](#rock-setting) ID references or inline rock setting objects.
