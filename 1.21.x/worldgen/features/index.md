...
layout: page
title: "Features"
permalink: /1.21.x/worldgen/features/
---
...
- Decoration:
  ...
  - [Creeping Ocean Plant](#creeping-ocean-plant)
  - [Rotatable Water Plant](#rotatable-water-plant)
  - [Epiphyte Plant](#epiphyte-plant)
  - [Kelp](#kelp)
  - [Kelp Tree](#kelp-tree)
  - [Submerged Tall Plant](#submerged-tall-plant)
...
- Config:
  ...
  - `allow_underwater`: A boolean specifying if the spikes can place underwater.
...
Places a column from floor to ceiling in caves. It uses the rock type of the current position.
...
- Config:
  ...
  - `radius`: (Default: `14`) An optional integer in the range [1, 16]. The approximate radius of the hot spring.
  - `decoration`: An optional [Fissure Decoration](#fissure) object.
  - `allow_underwater`: (Default: `false`) A boolean specifying if the hot spring can generate underwater.
  - `replaces_on_fluid_contact`: An optional map of blocks that can be replaced when the hot spring's fluid contacts them.
...
- Config:
  ...
  - `replace_fluids`: A list of fluid IDs that can be replaced by this lake.
...
### Sea Stacks
Places sea stack formations. It uses the rock type of the current position.
...
### Creeping Ocean Plant
Places a cylinder of creeping plants on blocks underwater.

- Type: `tfc:creeping_ocean_plant`
- Config:
  - `block`: A block id of a creeping plant.
  - `radius`: The radius of the cylinder.
  - `height`: The height of the cylinder.
...
### Rotatable Water Plant
Places a rotatable water plant that can attach to blocks in any direction.

- Type: `tfc:rotatable_water_plant`
- Config:
  - `block`: A rotatable water plant block to be placed.
...
### Submerged Tall Plant
Places a two block high plant submerged underwater.

- Type: `tfc:submerged_tall_plant`
- Config:
...
### Spreading Crop
Places a spreading crop with a fruit.

- Type: `tfc:spreading_crop`
- Config:
  - `block`: A wild spreading crop block to be grown.
...
Places bamboo plants in a cluster.

- Type: `tfc:bamboo`
- Config:
  - `probability`: A float [0, 1] specifying the probability of placing bamboo at any given location in the cluster.
  - `state`: A [Lenient Blockstate](../common-types/#lenient-blockstate) corresponding to a bamboo block.
