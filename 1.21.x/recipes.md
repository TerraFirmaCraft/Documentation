...
Alloy recipes are used in the creation of alloys in small vessels and crucibles. They contain a list of percentages of each metal which must be satisfied.
...
- `contents`: An array of objects, each containing the following properties:
  - `fluid`: (String) The registry name of the required metal fluid.
...
- `type`: `tfc:anvil`
- `ingredient`: An [Ingredient](../ingredients/), of the input item. Note that Anvil recipes can have multiple recipes with the same ingredient, and will be selected when the plan is selected for that item.
...
- `input_item`: An optional [Sized Ingredient](../ingredients/#sized), representing the input item. Defaults to empty.
- `input_fluid`: A optional [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredients), representing the input fluid. Defaults to empty.
...
- `primary_fluid`: A [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredient), representing the input fluid in the barrel.
- `added_fluid`: A [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredient), representing the fluid that must be added via a fluid container.
...
- `input_item`: An optional [Sized Ingredient](../ingredients/#sized), representing the input item. Defaults to empty.
- `input_fluid`: A optional [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredient), representing the input fluid. Defaults to empty.
- `output_item`: An optional [Item Stack Provider](../common-types/#item-stack-providers), representing the output item. Defaults to empty.
- `output_fluid`: A [Fluid Stack](../common-types/#fluid-stack), representing the output fluid. Defaults to empty.
- `sound`: A string, representing the registry name of a sound event, which is played when the recipe finishes. Defaults to `minecraft:block.brewing_stand.brew`.
...
- `fluid`: A [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredient) for the input fluid.
...
- `type`: `tfc:bloomery`
- `fluid`: A [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredient). The fluid that primary inputs must be able to melt into, to be considered primary inputs.
...
- `fluid`: A [Sized Fluid Ingredient](../common-types/#sized-fluid-ingredient). This ingredient is used to match the contents of the solidified mold.
...
- `mode`: The chisel mode that is required. Chisel modes are referenced by their registry name.
...
- `item_output`: An optional [Item Stack Provider](../common-types/#item-stack-providers) specifying an item to be dropped on chisel completion.
...
```json
{
  ...
  "mode": "tfc:slab",
  "result": "tfc:alabaster/bricks/black_slab"
}
```
...
**Note 2** You may not need to add a collapse recipe in most cases. Adding a block to the `tfc:can_collapse` tag without a recipe attached will cause it to be able to collapse, however it will not 'transform' into another block, and only match the block given in the tag entry.
...
```json
{
  ...
  "result": "tfc:ore/normal_bismuthinite/andesite"
}
```
...
The following operations are possible: `blow`, `roll`, `stretch`, `pinch`, `flatten`, `saw`, `table_pour`, `basin_pour`, `amethyst`, `soda_ash`, `sulfur`, `iron`, `ruby`, `lapis_lazuli`, `pyrite`, `sapphire`, `gold`, `graphite`, `copper`, `nickel`, `tin`, `silver`
...
**Note 2** You may not need to add a landslide recipe in most cases. Adding a block to the `tfc:can_landslide` tag without a recipe attached will cause it to be able to landslide, however it will not 'transform' into another block, and only match the block given in the tag entry.
...
```json
{
  ...
  "ingredient": [
    "tfc:clay/alfisol",
    "tfc:clay_grass/alfisol",
    "tfc:clay_duff/alfisol"
  ],
  "result": "tfc:clay/alfisol"
}
```
...
- `type`: `tfc:loom`
- `ingredient`: An [Sized Ingredient](../ingredients/#sized). This is the input for the recipe. Note that this is a Sized Ingredient, which means it specifies the count of items required in the ingredient. It is typical for loom recipes to require more than one item.
- `result`: An [Item Stack Provider](../common-types/#item-stack-providers). The result produced by this recipe.
- `steps`: An integer, which determines how many steps of the loom's working animation need to be completed to produce one product item.
- `texture`: The texture used in the loom rendering when this recipe is in progress.
...
Quern recipes are used for grinding items in the Quern. The handstone slot is special and will accept any item with the tag `tfc:quern_handstones` (See [Item Tags](../tags/#item-tags)). It has the following properties:
...
- `ingredient`: An [Ingredient](../ingredients/). This is the input for the recipe.
- `result`: An [Item Stack](../common-types/#item-stacks). The output of the recipe.
- `result_item`: An optional [Item Stack](../common-types/#item-stacks). An additional item output that is produced when the scraping is complete.
...
```json
{
  "type": "tfc:scraping",
  "ingredient": {
    "item": "tfc:large_sheepskin_hide"
  },
  "input_texture": "tfc:item/hide/large/sheepskin",
  "output_texture": "tfc:item/hide/large/raw",
  "result": {
    "count": 1,
...
Sewing recipes are used with the Sewing Table to create items from cloth. The recipe defines a pattern of colored cloth squares and stitches that must be matched. Sewing recipes have the following properties:

- `type`: `tfc:sewing`
- `result`: An [Item Stack](../common-types/#item-stacks). The output of the recipe.
- `squares`: An array of strings representing the pattern of colored cloth squares. Each character represents a square: 'W' for wool, 'B' for burlap. Spaces represent empty squares.
- `stitches`: An array of strings representing the stitch pattern. '#' represents a stitch, and spaces represent no stitch. The stitch pattern must match the dimensions of the squares pattern.
